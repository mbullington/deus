...
// MBULLINGTON: Inlined this from error.rs
#[derive(Debug)]
pub enum SudoStringError {
...
impl SudoString {
    pub fn new(mut string: String) -> Result<Self, SudoStringError> {
        if string.as_bytes().contains(&0) {
            return Err(SudoStringError::StringValidation(string));
        }
